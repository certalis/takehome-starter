...
1. See their sessions in a list, with date, location, attendee count, status.
2. Display financial analytics.
3. Cancel a session (with a reason).
4. Bulk-confirm sessions older than 7 days.
5. Filter sessions by status and date range.
6. Confirm a session.
7. Export the sessions to CSV.
8. Receive a toast when a new booking comes in (polling the API every 30s is fine).
9. Edit a session.
...
- **Option B — `NOTES.md` at the repo root**: write it down yourself — **≤ 60 lines, in your own words** — structured around the same 4 points. We read it to understand *your* reasoning, so write it, don't generate it.
...
- Trade-offs: where did you take shortcuts on purpose?
...
- Auto-seed on startup if the DB is empty (5 trainers, 120 sessions, 8 bookings)
